...
- Remove duplicated local validation by expanding CORE_VALID modes
- Add list-based stats workflows (beyond fixed-size input)
- Add robust output mode switching in CORE_RESULT

## Testing checklist
- Verify all menu loops return safely
- Verify no `Stop` usage in recoverable validation paths
- Verify each program sets TITLE and uses CORE_FORMAT
...
- Reuse constants and mode flags from CORE_CONST only
